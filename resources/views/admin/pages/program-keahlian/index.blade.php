        ...
        <table class="table table-striped table-sm datatable">
            <thead>
                <tr>
                    ...
                    <th width="25%">{{__('BIDANG KEAHLIAN')}}</th>
                ...
                @foreach($items as $val)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{ url('/app-admin/program-keahlian/'.encrypt($val->id).'/edit') }}" class="mx-1 text-dark">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a href="#" onclick="onDestroy('<?= url('/app-admin/program-keahlian/' . encrypt($val->id)) ?>', '{{ $val->nama }}')" class="mx-1 text-danger">
                                <i class="fas fa-trash"></i>
                            </a>
                        </td>
                        <td>{{ $val->kode }}</td>
                        <td>{{ $val->nama }}</td>
                        <td>{{ $val->nama_bidang_keahlian }}</td>
...
<script>
    ...
    function onHapusMassal(url) {
        ...
        Swal.fire({
            title: 'KONFIRMASI',
            text: 'Apakah anda yakin akan menghapus massal ?, semua Program Keahlian akan terhapus secara permanen !',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
