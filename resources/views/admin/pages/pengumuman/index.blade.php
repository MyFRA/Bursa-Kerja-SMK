                ...
                @foreach($items as $val)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{ url('/app-admin/pengumuman/'.encrypt($val->id).'/edit') }}" class="mx-1 text-dark">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a href="#" onclick="onDestroy('<?= url('/app-admin/pengumuman/' . encrypt($val->id)) ?>', '{{ $val->judul }}')" class="mx-1 text-danger">
...
<script>
    ...
    function onDestroy(url, nama) {
        Swal.fire({
            title: 'KONFIRMASI',
            text: 'Apakah anda yakin akan menghapus Pengumuman ' + nama + '?',
            icon: 'warning',
